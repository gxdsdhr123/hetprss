...
$(function() {
	...
	$('#createRule').click(function() {
		iframe = layer.open({
			...
			closeBtn : false,
			area:['100%','100%'],
			content : ctx + "/stand/gate/form?type=create&schemaId=4&id=",
			btn : [ "保存", "取消" ],
		...
		if(clickRowId==null){
			...
			var id = clickRowId;
				iframe = layer.open({
					...
					area:['100%','100%'],
					closeBtn : false,
					content : ctx + "/stand/gate/form?type=update&schemaId=4&id=" + id,
					btn : [ "保存", "取消" ],
					yes : function(index, layero) {
						var iframeWin = window[layero.find('iframe')[0]['name']];
						var submitData = iframeWin.getSubmitData();
	...
	var tableOptions = {
	    ...
	    columns: [
			...
			{field: "GATE_STR", title: "登机口",editable:false,halign:'center'}
	    ]
	};
