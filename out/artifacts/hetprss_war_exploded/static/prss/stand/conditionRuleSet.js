...
function doCheck(){
	...
	//规则名称放宽至50个中文汉字
	if( ! this.checkLength($("#ruleName").val(),100 ) ){//中文字符占两个长度
		showWarnMsg("规则名称不能超过50个中文字符！");
		return false ;
	}
	return true;
}
		...
		success : function(data) {
			if(data.code == 0){
				...
				$.each($("#selUnfixedUL li"),function(k,v){
					actstands.push($(v).data("code"));
				})
				paramsJsonObj['actstands'] = actstands.join(",");
			}else{
...
function addParam(){
	layer.open({
		type : 2,
		title : '编辑表达式',
		content : ctx + "/stand/condition/openParam",
		btn : [ "确定",  "取消" ],
		area : [ '800px', '450px' ],
		yes : function(index, layero) {
			var iframeWin = window[layero.find('iframe')[0]['name']];
			...
			$.ajax({
				type : 'post',
				url : ctx + "/stand/condition/saveParam",
				data : {
					data : submitData
				},
				async : true,
...
function initParam(list){
	...
	$.each(list,function(index,item){
		$("#params").append('<input type="button" value="'+item.COL_CNNAME +'" class="layui-btn layui-btn-small layui-btn-normal" '+
				'style="width: 120px;" data-en="'+item.COL_NAME +'" data-id="'+item.ID +'"onclick="setValue(this)"></input> ');
	})
}
