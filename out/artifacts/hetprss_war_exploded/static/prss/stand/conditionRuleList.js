...
$(function() {
	...
	$('#createRule').click(function() {
		iframe = layer.open({
			...
			closeBtn : false,
			area:['100%','100%'],
			content : ctx + "/stand/condition/form?type=create&id=",
			btn : [ "保存", "取消" ],
		...
		if(clickRowId==null){
			...
			var id = clickRowId;
				iframe = layer.open({
					...
					area:['100%','100%'],
					closeBtn : false,
					content : ctx + "/stand/condition/form?type=update&id=" + id,
					btn : [ "保存", "取消" ],
					yes : function(index, layero) {
						var iframeWin = window[layero.find('iframe')[0]['name']];
						var submitData = iframeWin.getSubmitData();
		...
		if(clickRowId==null){
			...
			var id = clickRowId;
				...
				// 按钮
					...
					$.ajax({
						type : 'post',
						url : ctx + "/stand/condition/delete",
						data : {
							ruleId : id
						},
						async : false,
	...
	baseTable = $("#baseTable");
	var tableOptions = {
		url: ctx + "/stand/condition/ruleList", 
	    method: "get",
	    dataType: "json",
		striped: true,
	    cache: true,
	    ...
	    columns: [
			...
			{field: "TYPE", title: "分配规则类型",editable:false,align:'left',halign:'center',formatter:function(value,row,index){
			...
			{field: "ACTSTAND_STR", title: "机位",editable:false,halign:'center'}
	    ]
	};
