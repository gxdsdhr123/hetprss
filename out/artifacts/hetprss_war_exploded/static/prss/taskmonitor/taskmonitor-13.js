...
layui.use(['layer','form'],function(){
	...
	form.on('select(inOrOut)',function(data){
		initParam.inOrOut = data.elem.value;
		refresh();
	});
						...
						$.ajax({
					        ...
					        success: function(msg){
					            if(msg == 'success'){
					            	// 任务块状态变更
									/*data.jobState = 6;
									var title = data.taskDescribe?data.taskDescribe:'';
									$(thisObj).html(title);
									$(thisObj).removeClass().addClass('drag-item').addClass('item-status-6');
									*//** 重新定义事件*//*
...
function showLegend(){
	...
	layer.open({
		type:2,
		title:"图例说明",
		content:ctx + '/taskmonitor/legend?schemaId=13',
		area:['1100px','600px'],
		btn:['关闭']
	});
}
