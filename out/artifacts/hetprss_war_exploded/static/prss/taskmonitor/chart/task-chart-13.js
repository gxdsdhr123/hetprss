(function ($) {
	...
	var itemwidth = 90; 
	// 方块宽度
	var personItemwidth = 110; 
	// 方块高度
	var itemHeight = 32;
	// 最多人员任务
	var max_person_item = 6;
	// 最多航班任务
	var max_flight_item = 6;
	// 航班区域title高度
	var flightTitleHeight = 100;
	// 航班区域高度
	var flightHeight = 320;
	// 人员区域高度
	var personHeight = 270;
	// 滚动条高度
	...
	var draw = function (options) {
		...
		// 画板大小
		$this.height(flightHeight+personHeight*2+scrollbarHeight*2);
		...
		var airportDiv1 = $('<div class="airport" id="airportDiv1"></div>');
		var airportDiv2 = $('<div class="airport" id="airportDiv2"></div>');
		...
		if(options.airport){
			$.each(options.airport,function(k,v){
				...
				if(persons){
					...
					// 样式
					airport_item.width(itemspace * persons.length + (v.name?areaTextWidth:0)).height(personHeight-2);
		...
		airportDiv1.width(itemspace * num1 + areaTextWidth * (title_num1-1)).height(personHeight-2).css('top',scrollbarHeight+flightHeight+2+'px');
		if(airportDiv1.width() < $this.width()){
			airportDiv1.width($this.width());
		}
		$this.append(airportDiv1);
		
		airportDiv2.width(itemspace * num2 + areaTextWidth * (title_num2-1)).height(personHeight-2).css('top',scrollbarHeight+flightHeight+personHeight+2+'px');
...
//		$this.append(middleScroll);
		...
		bottomScroll.height(scrollbarHeight).css('top',scrollbarHeight+flightHeight+personHeight*2+'px');
...
//					container.append(createDraggable(t.personName,t,type));
	...
	var createDraggable = function (title,data,type) {
		if(!title){
			...
			if((type == 'YP' || type == 'NP') && data.taskStyle){
				title = '<i>'+data.taskStyle+'</i>'+ title;
			}
		...
		if(type == 'YP' || type == 'NP'){
			content.width(personItemwidth).addClass('task-job-type');
		}
...
//		setScroll('middleScroll',['airport']);
		setScroll('bottomScroll',['airportDiv1','airportDiv2']);
//		
//		// 初始位置设置
...
//				'middleScroll':['airport'],
				'bottomScroll':['airportDiv1','airportDiv2']
		}
