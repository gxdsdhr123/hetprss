(function ($) {
	...
	// 航班区域title高度
	var flightTitleHeight = 116;
	// 航班区域高度
	var flightHeight = 236;
	// 人员区域高度
	...
	var draw = function (options) {
		...
		yFlightDiv.width($this.width()).height(flightHeight*2).css('top',0);
		...
		if(options.airport){
			$.each(options.airport,function(k,v){
				...
				if(persons){
					...
					if(npersons.length != ypersons.length){
						npersons.push({});
					}
					// 上边人员
					var title1 = $('<div class="airport-title">' + v.name + '</div>').height(areaTextHeight);
//					title1.on('dblclick',function(e){
//						if($(this).next().is(":hidden")){
//							$(this).nextAll().show();
//							$(this).parent().width(itemspace * ypersons.length)
//							$(this).parent().css("background-color","");
//						    sessionStorage.setItem(v.name,"0");  
//						}else{
//							$(this).nextAll().hide();
//							$(this).parent().width("24px");
//							$(this).parent().css("background-color","#173c76")
//							sessionStorage.setItem(v.name,"1");
//						}
//					});
					
					title1.css("cursor","default");
					airport_item.append(title1);
					$.each(ypersons,function(i,p){
						// 单个人员容器
					...
					// 下边人员
					/*var title2 = $('<div class="airport-title">&nbsp;</div>').width(areaTextWidth - 5);
					airport_item.append(title2);*/
					$.each(npersons,function(i,p){
						// 单个人员容器
						var container = createContainer(p , 'NP');
						airport_item.append(container);
					})
					// 样式
					airport_item.width(itemspace * ypersons.length).height(personHeight*2);
					//airport_item.find('.airport-title').width(itemspace * persons.length + areaTextWidth);
					airportDiv.append(airport_item);
					title_num++;
//					if( sessionStorage.getItem(v.name) == "1"){
//						$(title1).nextAll().hide();
//						$(title1).parent().width("24px");
//						$(title1).parent().css("background-color","#173c76")
//					}else{
//						$(title1).nextAll().show();
//						$(title1).parent().width(itemspace * ypersons.length)
//						$(title1).parent().css("background-color","");
//					}
				}
			});
		}
		
		airportDiv.width(itemspace * num + 20 * title_num).height(personHeight*2 + areaTextHeight + 4).css('top',flightHeight*2+'px');
...
//		
	...
	var createContainer = function (data , type){
		...
		if(type == 'YF' || type == 'NF'){
			...
			title.append('<h4>' + (data.acttypeCode?data.acttypeCode:'———') + '</h4>');	//出港航班号 
			title.append('<span class="title-time">' + (data.inTime?data.inTime:'----')+' | '+(data.outTime?data.outTime:'----') + '</span>');	// 进/出港显示时间
			title.append( '<br/>'+(data.seat?data.seat:'')+' | '+(data.bagCrsl?data.bagCrsl:''));	// 机位
