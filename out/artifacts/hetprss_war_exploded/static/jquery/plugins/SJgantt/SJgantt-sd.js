(function ($) {
	...
	function handleYData(result){
		tempYpool = {};
		isBusy = false;
		settings.yAxis.width = oriYWidth;
    	for(var i=0;i<result.length;i++){
    		var f = new field(result[i]);
    		if(f.inPlan == "0" && f.online == "1"){
    			f.name += "(补)";
    		}
    		if(f.car){
    			f.name += "("+f.car+")";
    		...
    		if(tempYpool[f.id]){
    			tempYpool[f.id+f.pid] = f;
    		}else{
    			tempYpool[f.id] = f;
    		}
        	if(f.level == "2" && f.pid != "所有航空公司" && f.pid != "未分配" && f.pid.indexOf("全机型")==-1 && f.pid.indexOf("全机位")==-1){
        		isBusy = true;
	...
	var field = function(f){
		this.blockup = f.blockup?f.blockup:"0";
		this.id = f.id;
		this.pid = f.pid;
		this.name = f.name;
		this.level = f.level;
		this.inPlan = f.inPlan;
	...
	/*overlay.prototype = {
		validate:function(){
			if(this.field != "DP" && this.field != "QX"){
	...
	function calcExpand(f){
		ypool[f].list = [];
		for(var o in pool[f]){
			var oStart = pool[f][o].x;
			var oEnd = pool[f][o].x2;
			var textLength = cxt.measureText(pool[f][o].text).width;
			if(oStart+textLength+0.7*ypool[f].h+2 > oEnd){
				oEnd = oStart+textLength+0.7*ypool[f].h+2;
			}
			if(ypool[f].list.length==0){
				ypool[f].list.push([[oStart,oEnd]]);
				...
				for(var j=0;j<ypool[f].list.length;j++){
					...
					for(var k=0;k<ypool[f].list[j].length;k++){
						if(oStart<ypool[f].list[j][k][1] && ypool[f].list[j][k][0]<oEnd){
							colClash = true;
							break;
						}
					}
					if(!colClash){
						ypool[f].list[j].push([oStart,oEnd]);
    ...
    var addEvent = function(){
        				...
        				for(var y in ypool){
                			if(ypool[y].level=="2"){
                    			if(mouse.x>ypool[y].x && mouse.x<ypool[y].x+oriYWidth && mouse.y>ypool[y].y && mouse.y<ypool[y].y+ypool[y].h){
                    				...
                    				$("#call").data("id",ypool[y].id);
                    				if(ypool[y].blockup&&ypool[y].blockup==1){
                    					$("#stop").text("恢复");
                    				} else {
                    					$("#stop").text("停用");
                    				}
        			...
        			if(pool[f] && mouse.y>ypool[f].y && mouse.y<ypool[f].y+ypool[f].h){
        				for(var i=pool[f].length-1;i>=0;i--){
            	    		...
            	    		if(mouse.x>o.x && mouse.x<o.x2 && mouse.y>o.y && mouse.y<o.y+o.h){
            	    			...
            	    			if(wthMode && o.color!=colorPool.gray && o.color!=colorPool.red){
            	    				return false;
            	    			}
                	    					...
                	    					if(result != "success"){
                            	    			...
                            	    			$.ajax({
                            	    				...
                            	    				success:function(list){
                            	    					...
                            	    					if(o.field == "DP" || wthMode || o.ptId == "start" || !o.ptId){
                            	    						ypool["DP"].allow = true;
                            	    					}
                            	    					if(!wthMode){
                            	    						ypool["QX"].allow = true; 
    											...
    											if(tempO.field == "DP"){
    			    								if(yItem.id == "QX"){
    			    									...
    			    									if(!wthMode){
    			    										...
    			    										$.ajax({
        			    										type:'post',
        			    										url:ctx+"/scheduling/gantt/wthAllocationTask",
        			    										data:{
        			    											targetActor: yItem.id,
        			    											jobTaskId : tempO.id
        			    										},
    			    								...
    			    								if(yItem.id == "QX"){
    			    									...
    			    									if(!wthMode){
    			    										if(yItem.id == "DP"){
    			    											layer.confirm('确认还原任务吗？', {icon: 3, title:'提示'}, function(index){
    	    			    										$.ajax({
    	        			    										type:'post',
    	        			    										url:ctx+"/scheduling/gantt/restoreTask",
    	        			    										data:{
    	        			    											jobTaskId:tempO.id,
    	        			    											procId:tempO.proc,
    	        			    											orderId:tempO.order_id
    	        			    										...
    	        			    										success:function(data){
    	        			    											if(data=="succeed"){
    	        			    												layer.msg("任务已还原！",{icon:1,time:600},function(){
    	        			    													loadData();
    	        			    												});
    	        			    											} else {
    	        			    												layer.msg("还原失败",{icon:7});
    	        			    											}
    	        			    											recoverJmSemaState();
    	        			    										}
    	        			    									});
    			    										...
    			    										if(yItem.id == "DP"){
    			    											$.ajax({
            			    										type:'post',
            			    										url:ctx+"/scheduling/gantt/wthResetTask",
            			    										data:{
            			    											jobTaskId : tempO.id
            			    										},
            			    										success:function(data){
    	...
    	canvas.addEventListener('dblclick',function(e){
    		if(mouse.x>settings.yAxis.width+50 && mouse.x<canvas.width-50 && mouse.y>0 && mouse.y<settings.xAxis.height){
    			...
    			for(var f in ypool){
    				if(isBusy && (ypool[f].id=="DP" || ypool[f].id=="QX")){
    				...
    				}else if(ypool[f].level == "2" || ypool[f].id=="DP" || ypool[f].id=="QX"){
	...
	var drawOverlay = function(x,y,w,h,color,text,icon,status,isOdrag){
		cxt.beginPath();
		if(isOdrag || (wthMode && color!=colorPool.gray && color!=colorPool.red)){
			cxt.globalAlpha = 0.7;
		}else{
			cxt.globalAlpha = 1;
		}
		...
		//if(w2>length+0.7*h+4){
			...
			if(texts.length == 3){
				...
				cxt.font = "bolder 13px sans-serif";
				cxt.fillText(texts[1],x+0.7*h+2+width0,y+h/2);
				var width1 = cxt.measureText(texts[1]).width;
				cxt.font = "12px sans-serif";
				cxt.fillText(texts[2],x+0.7*h+2+width0+width1,y+h/2);
			}else{
	...
	var drawYAxis = function(type,code){
    	...
    	for(var f in ypool){
    		...
    		if(!isBusy && ypool[f].level == "1" && ypool[f].id != "DP" && ypool[f].id != "QX"){
    			continue;
    		}
    		var item = ypool[f];
    		var start = isBusy?item.level:item.level-1;
    		if(ypool[f].id == "DP" || ypool[f].id == "QX"){
    			start = item.level
    		}
    		...
    		//停用人员变红
    		if(item.blockup&&item.blockup==1){
    			cxt.fillStyle = colorPool.red.back;
    		} else {
    			if(item.online == "1"){
    				cxt.fillStyle = "#23C82E";
    			}else{
    				cxt.fillStyle = settings.yAxis.color;
    			}
    		}
			...
			if(dragO && (item.level=="2" || item.id=="DP" || item.id=="QX")){
				if(item.allow){
