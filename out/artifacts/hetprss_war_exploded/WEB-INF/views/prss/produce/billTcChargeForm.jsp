...
<body>
	<div id="container">
		<div id="baseTables">
			<form id="tableForm"  class="layui-form" action="${ctx}/produce/tcCharge/save"
				method="post">
				<table id="baseTable" class="layui-table"
					style="width: 900px; text-align: center; margin: 10px auto;">
					<thead>
						<tr>
							<th colspan=6>特车收费单</th>
						</tr>
					</thead>
					<tbody>
						<tr>
						...
						<tr>
							<td>服务项目</td>
							<td>
								<select id="servicePos" name="servicePos" >
									<option value="">请选择</option>
									<c:forEach items="${restype}" var="item">
										<option <c:if test="${result.SERVICE_POS==item.ID}">selected</c:if> value="${item.ID}" >${item.TEXT}</option>
									</c:forEach>
								</select>
							</td>
							<td>开始时间</td>
						...
						<tr>
							...
							<td><input type='text' name="duration"
								id="duration" class='form-control' lay-verify="numberByCus"
								value="${result.DURATION}"  /></td>
						</tr>
						<!-- 							6 -->
						<tr>
							<td>架次</td>
...
<!-- 								</select>	 -->
						...
						<tr>
							...
							<td>机组签字</td>
							<td>
								<img src="${ctx }/produce/special/outputPicture?id=${result.SIGN}" height=45 width=80/>
<%-- 								<img style="width:300;height:40px;" src="data:image/png;base64,'${result.SIGN}'" /> --%>
	...
	<script type="text/javascript"
		src="${ctxStatic}/prss/produce/billTcChargeForm.js"></script>
</body>
</html>