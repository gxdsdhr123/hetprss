...
import com.neusoft.prss.scheduling.dao.SchedulingProgressDao;
@Service
@Transactional(readOnly = true)
public class SchedulingProgressService extends BaseService {
	@Autowired
	private SchedulingProgressDao progressDao;
	...
	public JSONArray getTaskList(Map<String, Object> params) {
		...
		if(!result.isEmpty()){
			...
			for (int i = 0; i < result.size(); i++) {
				...
				fltids.add(fltInfo.getString("fltid"));
			...
			if (!taskAlarms.isEmpty()) {
				...
				for (int i = 0; i < result.size(); i++) {
					JSONObject fltInfo = result.getJSONObject(i);
					String fltid = fltInfo.getString("fltid");
					if (taskAlarmMap.containsKey(fltid)) {
						JSONArray alarms = taskAlarmMap.get(fltid);
						int alarm1 = 0;//一级预警
						int alarm2 = 0;//二级预警
						int alarm3 = 0;//三级预警
