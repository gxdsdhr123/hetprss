...
import com.neusoft.prss.statisticalanalysis.service.SaFltNumService;

/**
 * 航班架次统计表
 */
@Controller
@RequestMapping(value = "${adminPath}/saFltNum")
public class SaFltNumController extends BaseController{
	@Autowired
	private SaFltNumService saFltNumService;
	...
	public String list(Model model){
		return "prss/statisticalanalysis/saFltNumList";
	}
	...
	public JSONObject getData(HttpServletRequest request){
		...
		try {
			rs = saFltNumService.getDataList(params);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void printList(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			String fileName = "航班架次统计表" + startDate+"-"+endDate+ ".xlsx";
			setHeader(request, response, fileName);
			byte[] content = saFltNumService.getExcelByte(params);
			response.getOutputStream().write(content);
            response.getOutputStream().flush();
		} catch (Exception e) {
			e.printStackTrace();
			logger.error("航班架次统计表" + e.getMessage());
		}
	}
