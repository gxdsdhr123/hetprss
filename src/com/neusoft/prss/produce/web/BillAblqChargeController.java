...
import com.neusoft.prss.produce.service.BillAblqChargeService;
import com.neusoft.prss.produce.service.SafetyInspectionTestService;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/produce/ablqCharge")
public class BillAblqChargeController extends BaseController {
	
	@Autowired
	private BillAblqChargeService BillAblqChargeService;
	
	@Autowired
	private SafetyInspectionTestService safetyInspectionTestService;
	...
	public String list(Model model) {
		//System.out.println(safetyInspectionTestService.test());
		return "prss/produce/billAblqChargeList";
	}
	
	@ResponseBody
	@RequestMapping(value = "dataList")
	public String getDataList() {
		JSONArray json = BillAblqChargeService.getDataList();
	...
	public String add(Model model,String type) {
		JSONArray resTypeArr=BillAblqChargeService.getResType();
		String userid=UserUtils.getUser().getId();
		String username=UserUtils.getUser().getName();
//		JSONArray userArr=BillAblqChargeService.getSysUser();
		JSONObject result = new JSONObject();
		result.put("OPERATOR", UserUtils.getUser().getId());
...
//		model.addAttribute("userArr",userArr);
		...
		return "prss/produce/billAblqChargeForm";
	}
	
	@RequestMapping(value = { "edit" })
	public String edit(Model model,String type,String id) {
	    JSONArray billArr=BillAblqChargeService.getBillById(id);
	    JSONArray userArr=BillAblqChargeService.getSysUser();
	    JSONArray resTypeArr=BillAblqChargeService.getResType();
	    if(billArr.size()==1){
	...
	public ResultByCus getFltInfo(String flightDate,String flightNumber,String inOutFlag) {
		...
		JSONArray jsonArr = BillAblqChargeService.getFltInfo(param);
	...
	@RequestMapping(value = "save" )
		public ResultByCus save(BillAblqChargeEntity entity) {
		boolean flag=false;
		ResultByCus result=new ResultByCus();
		String userId=UserUtils.getUser().getId();
		...
		if ("add".equals(entity.getType())){
			flag=BillAblqChargeService.saveAdd(entity);
		}else if ("edit".equals(entity.getType())){
			flag=BillAblqChargeService.saveEdit(entity);
		}
		if(flag){
			result.setCode("0000");
			result.setMsg("操作成功");
	...
	@RequestMapping(value =  "del" )
		...
		ResultByCus result=new ResultByCus();
		boolean flag=BillAblqChargeService.delBillById(id);
		if(flag){
			result.setCode("0000");
			result.setMsg("操作成功");
		}else{
	    ...
	    public void printData(HttpServletRequest request,HttpServletResponse response) throws URISyntaxException{
		 ...
		 String wiPath = new File(Thread.currentThread().getContextClassLoader().getResource("").toURI()).getParentFile().getPath();
			...
			if (file != null && file.exists()) {
				JSONObject json = (JSONObject) BillAblqChargeService.getExportWordData(id).get(0);
	...
	public void printList(HttpServletRequest request, HttpServletResponse response,String startDate,String endDate) {
		...
		String[] title = { "日期", "航班号", "航空公司", "机型","机号","服务项目", "牵引车架次", "服务时长", "操作人", "备注" };
		JSONArray columnArr = JSON.parseArray(columnName);
		try {
			String fileName = "廊桥收费单" + DateUtils.getDate("yyyyMMddHHmmss") + ".xlsx";
			setHeader(request, response, fileName);
			ExportExcel excel = new ExportExcel(fileName);
			excel.setDataList(title, columnArr, BillAblqChargeService.getDataTotal(startDate,endDate));
			excel.write(response);
			excel.dispose();
			return;
		} catch (Exception e) {
			logger.error("廊桥收费单" + e.getMessage());
		}
	}
