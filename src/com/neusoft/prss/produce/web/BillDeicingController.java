...
import com.neusoft.prss.produce.service.BillDeicingService;
import com.neusoft.prss.stand.entity.ResultByCus;

@Controller
@RequestMapping(value = "${adminPath}/produce/deicing")
public class BillDeicingController extends BaseController {
	
	@Autowired
	private BillDeicingService billDeicingService;
	@Autowired
	private FileService fileService;
	
	@RequestMapping(value ="list")
	public String list(Model model) {
		return "prss/produce/billDeicingList";
	}
	...
	public String getDataList(String searchStr,String searchDate) {
		...
		JSONArray json = billDeicingService.getDataList(map);
...
//		model.addAttribute("userArr",userArr);
	...
	public ResultByCus getFltInfo(String flightDate,String flightNumber,String inOutFlag) {
		...
		JSONArray jsonArr = billDeicingService.getFltInfo(param);
	...
	@RequestMapping(value = "save" )
		public ResultByCus save(BillDeicingEntity entity) {
		boolean flag=false;
		ResultByCus result=new ResultByCus();
		String userId=UserUtils.getUser().getId();
		...
		if ("add".equals(entity.getType())){
			flag=billDeicingService.saveAdd(entity);
		}else if ("edit".equals(entity.getType())){
			flag=billDeicingService.saveEdit(entity);
		}
		if(flag){
			result.setCode("0000");
			result.setMsg("操作成功");
	...
	@RequestMapping(value =  "del" )
		...
		ResultByCus result=new ResultByCus();
		boolean flag=billDeicingService.delBillById(id);
		if(flag){
			result.setCode("0000");
			result.setMsg("操作成功");
		}else{
	...
	public void printData(HttpServletRequest request,HttpServletResponse response) throws URISyntaxException{
	 ...
	 String tempPath = "";
	 JSONObject json = (JSONObject) billDeicingService.getExportWordData(id).get(0);
	 if (json != null && !json.isEmpty()) {
		...
		@SuppressWarnings("unchecked")
			...
			if(json.containsKey("SIGN")&&json.get("SIGN")!=null){
				...
				tempPath = wiPath + "/template/BillDeicingTemplet.docx";
			}else{
				tempPath = wiPath + "/template/BillDeicingSignMarkTemplet.docx";
			}
	...
	public void printList(HttpServletRequest request, HttpServletResponse response,String startDate,String endDate) {
		...
		String columnName = "[{\"field\":\"FLIGHT_DATE\"},{\"field\":\"FLIGHT_NUMBER\"},{\"field\":\"AIRLINE_SHORTNAME\"},"
				+"{\"field\":\"AIRCRAFT_NUMBER\"},{\"field\":\"STATION\"},{\"field\":\"DEFID_DOSAGE\"},"
				+ "{\"field\":\"AIFID_DOSAGE\"},{\"field\":\"OPERATOR\"}]";
		String[] title = { "日期", "航班号", "航空公司", "机号","航站", "除冰液用量", "防冰液用量", "操作人" };
		JSONArray columnArr = JSON.parseArray(columnName);
		try {
			Map<String, Object> map = new HashMap<String, Object>();
			map.put("startDate", startDate);
			map.put("endDate", endDate);
			String fileName = "飞机除/防冰收费单据" + DateUtils.getDate("yyyyMMddHHmmss") + ".xlsx";
			setHeader(request, response, fileName);
			ExportExcel excel = new ExportExcel(fileName);
			excel.setDataList(title, columnArr, billDeicingService.getDataTotal(map));
			excel.write(response);
			excel.dispose();
			return;
		} catch (Exception e) {
			logger.error("飞机除/防冰收费单据" + e.getMessage());
		}
	}
