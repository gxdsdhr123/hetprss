 ...
 */
package com.neusoft.framework.modules.sys.service;
...
import com.neusoft.framework.common.security.Digests;
...
import com.neusoft.framework.common.service.ServiceException;
import com.neusoft.framework.common.utils.CacheUtils;
import com.neusoft.framework.common.utils.Encodes;
import com.neusoft.framework.common.utils.StringUtils;
import com.neusoft.framework.common.web.Servlets;
import com.neusoft.framework.modules.sys.dao.MenuDao;
import com.neusoft.framework.modules.sys.dao.RoleDao;
import com.neusoft.framework.modules.sys.dao.UserDao;
import com.neusoft.framework.modules.sys.entity.Menu;
import com.neusoft.framework.modules.sys.entity.Office;
import com.neusoft.framework.modules.sys.entity.Role;
import com.neusoft.framework.modules.sys.entity.User;
import com.neusoft.framework.modules.sys.utils.LogUtils;
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public void deleteUser(User user) {
		userDao.delete(user);
		//删除人员班组
		userDao.deleteUserFromGroup(user.getId());
		//删除人员作业组
		userDao.deleteUserFromTeam(user.getId());
		//删除资质限制
		userDao.deleteUserFromAptitudeLimit(user.getId());
		//删除资质
		userDao.deleteUserFromAptitudeInfo(user.getId());
		//删除分工限制
		userDao.deleteUserFromDivisionLimit(user.getId());
		//删除分工明细
		userDao.deleteUserFromDivisionDetail(user.getId());
		//删除分工
		userDao.deleteUserFromDivisionInfo(user.getId());
		//删除分工模板限制
		userDao.deleteUserFromDivisionTempLimit(user.getId());
		//删除分工模板
		userDao.deleteUserFromDivisionTemp(user.getId());
		//删除人员排班
		userDao.deleteUserFromWorkerPlan(user.getId());
		//删除设备绑定
		userDao.deleteVehicleBound(user.getId());
		// 同步到Activiti
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	...
	public String getWorkerLevel(String id) {
		List<Map<String,String>> res = userDao.getWorkerLevel(id);
		if(res.isEmpty()) {
			return "-1";
		}else {
			return res.get(0).get("FLAG");
		}
	}

	public String getIfLeader(String id) {
		List<Map<String,String>> res = userDao.getIfLeader(id);
		if(res.isEmpty()) {
			return "-1";
		}else {
			return res.get(0).get("FLAG");
