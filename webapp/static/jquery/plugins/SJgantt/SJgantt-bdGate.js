...
//		alert(JSON.stringify(result));
	...
	function handleData(result){
		...
		for(var i in result){
			...
			if(tempYpool[o.issue_gate]){//判断Y轴项是否存在
				if(!tempPool[o.issue_gate]){//判断覆盖物池，该属性是否存在
					tempPool[o.issue_gate] = [];
				}
				
				var text =o.fltNumber+""+(o.issue_gate=="DP"?'':"("+o.issue_gate+")");
				o.text = text;
				//定义未保存的临时块
				if(o.issue_gate != o.save_gate){
					var child = {};
					for(var attr in o){
						child[attr] = o[attr];
					}
					child.issue_gate = o.save_gate;
					child.isChild = true;
					child.oriStand = o.issue_gate;
					if(!tempPool[child.issue_gate]){
						tempPool[child.issue_gate] = [];
					}
					tempPool[child.issue_gate].push(child);
					o.hasChild = true;
				}
				tempPool[o.issue_gate].push(o);
			}
		}
		ypool = tempYpool;
		pool = tempPool;
	...
	var overlay = function(d){
		...
		this.issue_gate = d.issue_gate;
		this.save_gate = d.save_gate;
		this.ifLeaved=d.ifLeaved;
		this.start = d.start;
		this.end = d.end;
		this.fltNumber=d.fltNumber;
		this.color = colorPool.gray;
		this.clash = false;
...
//            	    		if(!o.lock && !o.hasChild && mouse.x>o.x && mouse.x<o.x2 && mouse.y>o.y && mouse.y<o.y+o.h){
            	    		if(!o.hasChild && mouse.x>o.x && mouse.x<o.x2 && mouse.y>o.y && mouse.y<o.y+o.h){
//            	    			if(o.field == "QX" ){
//            	    				return false;
//            	    			}
            	    			if(o.ifLeaved == "1" ){
    	...
    	canvas.addEventListener('mouseup',function(e){
    		draggingY = false;
    		if(dragO&&dragO.ifLeaved!="1"){
    			for(var k in ypool){
    				var dragOY = mouse.y;
					if(dragOY>ypool[k].y && dragOY<ypool[k].y+ypool[k].h){
						var yItem = ypool[k];
						if(k!=dragO.issue_gate){
							var tempO = dragO;
							if(yItem.id == "DP"){
//								layer.confirm('确定置空登机口吗？', {icon: 3, title:'提示'}, function(index){
									var loadIcon = layer.load(1);
								
									$.ajax({
										type:'post',
										url:ctx+"/flightdynamic/gantt/removeGate",
										data:{
											fltid:tempO.fltid
										},
										success:function(data){
											if(data=="success"){
												layer.msg("登机口已置空！",{icon:1,time:600},function(){
													loadData();
												});
											} else {
...
//								layer.confirm('确定分配登机口吗？', {icon: 3, title:'提示'}, function(index){
									var loadIcon = layer.load(1);
									$.ajax({
										type:'post',
										url:ctx+"/flightdynamic/gantt/setGate",
										data:{
											fltid : tempO.fltid,
											gate: yItem.id
										},
										success:function(data){
											if(data=="success"){
												layer.msg("登机口已分配！",{icon:1,time:600},function(){
													loadData();
												});
											} else {
...
//	    if(o.type == "0"){//正常航班
			...
			cxt.beginPath();
			cxt.lineWidth = 4;
			cxt.strokeStyle = o.clash?colorPool.yellow.border:o.color.border;
			cxt.moveTo(o.x,o.y);
			...
			if(o.hasChild){
				for(var i=0;i<pool[o.save_gate].length;i++){
					if(pool[o.save_gate][i].id == o.id){
						var child = pool[o.save_gate][i];
						if(o.y<child.y){
							var line = {
									x1:o.x+o.w/2,
									y1:o.y+o.h,
...
//		for(var i in result){
//			var o = new overlay(result[i]);
//			if(tempYpool[o.save_gate]){//判断Y轴项是否存在
//				if(!tempPool[o.save_gate]){//判断覆盖物池，该属性是否存在
//					tempPool[o.save_gate] = [];
//				}
//				
//				var text =o.fltNumber+""+(o.issue_gate=="DP"?'':"("+o.issue_gate+")");
//				o.text = text;
//				//定义未保存的临时块
//				if(o.save_gate != o.issue_gate){
//					var child = {};
//					for(var attr in o){
//						child[attr] = o[attr];
//					}
//					child.issue_gate = o.save_gate;
//					child.isChild = true;
//					child.oriStand = o.save_gate;
//					o.save_gate = o.issue_gate;
//					if(!tempPool[child.save_gate]){
//						tempPool[child.save_gate] = [];
//					}
//					tempPool[child.save_gate].push(child);
//					o.hasChild = true;
//				}
//				tempPool[o.save_gate].push(o);
//			}
//		}
//		ypool = tempYpool;
//		pool = tempPool;
