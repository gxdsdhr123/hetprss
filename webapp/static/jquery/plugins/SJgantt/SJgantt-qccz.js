(function ($) {
	...
	var isBusy = false;//是否为忙时（分工不为全部航空公司）
	...
	var methods = {
	        init: function (options) {
	            ...
	            settings.yAxis.width += settings.yAxis.width;
	            this.css("width","100%");
	...
	function handleYData(result){
    	...
    	for(var i=0;i<result.length;i++){
        	...
        	ypool[f.id] = f;
        	if(!isBusy && f.level == "2" && f.pid != "所有航空公司" && f.pid != "未分配" && f.pid.indexOf("全机型")==-1 && f.pid.indexOf("全机位")==-1){
        		isBusy = true;
        		settings.yAxis.width = 3*oriYWidth;
        	}
        	if(ypool[f.pid] && ypool[f.pid].isLeaf){
        		ypool[f.pid].isLeaf = false;
        	}
        };
	}
    ...
    //加载数据
	function loadData(){
		var interval = sessionStorage[$("#loginName").val()+$("#schemaId").val()+"refreshGantt"] || "0.5";
		$.ajax({
    		type:'post',
    		url:settings.url,
    		async:false,
    		...
    		beforeSend:function(){
				$("#refresh").html("<i class='fa fa-spinner' style='display:inline-block;width:15px'>&nbsp;</i>每" + interval + "分钟");
			},
			error:function(){
				$("#refresh").text("刷新失败");
			},
	...
	var overlay = function(d){
		...
		this.proc = d.proc;
		this.fltid = d.fltid;
		this.in_fltid = d.in_fltid;
		this.out_fltid = d.out_fltid;
		...
		if(d.start){
			...
			this.x = time2Coor(d.estart);
		}
		if(d.end){
			this.x2 = time2Coor(d.end);
		}else if(d.eend){
			this.x2 = time2Coor(d.eend);
		}
		this.y = ypool[this.field]?ypool[this.field].y:-50;
		this.w = this.x2 - this.x;
		this.h = (d.color=="gray"||d.color=="green"?0.6:0.8)*settings.grid.lineHeight|0;
		this.color = colorPool[d.color];
		this.ocolor = colorPool[d.color];
		this.text = d.text;
		this.icon = d.icon;
		this.page = 1;
		this.detail = [[d.text]];
	...
	overlay.prototype = {
		validate:function(){
			if(this.field.indexOf("DP") == -1 && this.field.indexOf("QX") == -1){
				for(var o in pool[this.field]){
					if(this.x<pool[this.field][o].x2 && pool[this.field][o].x<this.x2){
						this.clash = true;
						this.color = colorPool.yellow;
						this.h = 0.8*settings.grid.lineHeight|0;
						pool[this.field][o].clash = true;
						pool[this.field][o].color = colorPool.yellow;
						pool[this.field][o].h = 0.8*settings.grid.lineHeight|0;
					}
	    		}
			}
		}
	...
	function calcY(){
		...
		ypool["JWQCCZZgnqc"].y = tempY;
		ypool["JWQCCZZgnqc"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCCZZgnqc",tempY);
		ypool["JWQCCZZgnqc"].h = tempY-ypool["JWQCCZZgnqc"].y; 
		ypool["JWQCCZZgjqc"].y = tempY;
		ypool["JWQCCZZgjqc"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCCZZgjqc",tempY);
		ypool["JWQCCZZgjqc"].h = tempY-ypool["JWQCCZZgjqc"].y; 
		ypool["JWQCCZZsdqc"].y = tempY;
		ypool["JWQCCZZsdqc"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCCZZsdqc",tempY);
		ypool["JWQCCZZsdqc"].h = tempY-ypool["JWQCCZZsdqc"].y; 
		ypool["JWQCCZZbp"].y = tempY;
		ypool["JWQCCZZbp"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCCZZbp",tempY);
		ypool["JWQCCZZbp"].h = tempY-ypool["JWQCCZZbp"].y; 
		contentHeight = tempY - basicY;
	}
	function calcCustomY(type,tempY){
		ypool[type+"DP"].y = tempY;
		ypool[type+"DP"].h = settings.grid.lineHeight;
		if(ypool[type+"DP"].expand){
			calcExpand(type+"DP");
		}else{
			giveY(type+"DP",ypool[type+"DP"].y);
		}
		tempY += ypool[type+"DP"].h; 
		ypool[type+"QX"].y = tempY;
		ypool[type+"QX"].h = settings.grid.lineHeight;
		if(ypool[type+"QX"].expand){
			calcExpand(type+"QX");
		}else{
			giveY(type+"QX",ypool[type+"QX"].y);
		}
		tempY += ypool[type+"QX"].h;
		for(var r in yDataResult){
			if(ypool[yDataResult[r].id].isLeaf && yDataResult[r].level == "1" || yDataResult[r].level== "0"){
				continue;
			}
			var f = yDataResult[r].id;
			if(f != type+"DP" && f != type+"QX" && yDataResult[r].level != "0"){
				if(isBusy){
					if(ypool[f].level == "1" && f.indexOf(type)!=-1){
						ypool[f].y = tempY;
						for(var s in ypool){
							if(ypool[s].pid == ypool[f].id){
								ypool[s].y = tempY;
					...
					settings.yAxis.width = 2*oriYWidth;
					if(ypool[f].level == "2"  && f.indexOf(type)!=-1){
						ypool[f].x = oriYWidth;
						ypool[f].y = tempY;
	...
	function calcX(){
		for(var f in pool){
			for(var o in pool[f]){
				...
				}else if(pool[f][o].estart){
					pool[f][o].x = time2Coor(pool[f][o].estart);
				}
				if(pool[f][o].end){
					pool[f][o].x2 = time2Coor(pool[f][o].end);
				}else if(pool[f][o].eend){
					pool[f][o].x2 = time2Coor(pool[f][o].eend);
				}
    ...
    var addEvent = function(){
    	...
    	canvas.addEventListener('mousedown',function(e){
    		...
    		if(mouse.x<settings.yAxis.width){
    			...
    			if(mouse.y>settings.xAxis.height){
        				...
        				for(var y in ypool){
                			if(ypool[y].level=="2"){
                    			if(mouse.x>ypool[y].x && mouse.x<ypool[y].x+oriYWidth && mouse.y>ypool[y].y && mouse.y<ypool[y].y+ypool[y].h){
                    				...
                    				$("#stopMenu").show();
                    				$("#stop").data("id",ypool[y].id+"@"+ypool[y].name);
                    				$("#member").data("id",ypool[y].id);
                    			}
                			}
                		}
        			}
    				...
    				settings.yData.queryParams.type = types[typeIndex];
    				loadYData();
    				basicY = 0;
    				calcY();
    			}
            	    		...
            	    		if(mouse.x>o.x && mouse.x<o.x2 && mouse.y>o.y && mouse.y<o.y+o.h){
            	    			...
            	    			moveDelay = setTimeout(function(){
            	    				$.ajax({
                	    				...
                	    				success:function(result){
                	    					if(result != "success"){
                	    						layer.msg(result,{icon:7});
                	    					}else{
                	    						$(canvas).css("cursor","move");
                            	    			dragO = o;
                            	    			var startTime = o.estart;
                            	    			var endTime = o.eend;
                            	    			if(o.start && !o.end){
                            	    				var cm = ((new Date(o.eend).getTime()-new Date(o.estart).getTime())-(new Date().getTime()-new Date(o.start).getTime()))/60000;
                            	    				if(cm>0){
                            	    					startTime = calcTime(new Date(),0)+":00";
                                	    				endTime = calcTime(new Date(),cm)+":00";
                            	    				}
                            	    			}
                            	    			$.ajax({
                            	    				type:'post',
                            	    				url:ctx+"/scheduling/gantt/manualSelectDriver",
                            	    				delay:200,
                            	    				data:{
                            	    					fltId : o.fltid,
                            	    					jobType : o.type,
                            	    				...
                            	    				success:function(list){
                            	    					...
                            	    					$.each(list,function(i,item){
                            	    						if(ypool[o.type+item]){
                            	    							ypool[o.type+item].allow = true;
                            	    						}
                            	    					})
                            	    					if(o.field.indexOf("DP") != -1){
                            	    						ypool[o.type+"DP"].allow = true;
                            	    					}
                            	    					ypool[o.type+"QX"].allow = true;
                            	    				}
                            	    			});
                	    					}
                	    				}
    	...
    	canvas.addEventListener('mouseup',function(e){
    		...
    		if(dragO){
    			for(var k in ypool){
    				if(ypool[k].level == "2" || k.indexOf("DP")!=-1 || k.indexOf("QX")!=-1){
    					var dragOY = mouse.y;
    					if(dragOY>ypool[k].y && dragOY<ypool[k].y+ypool[k].h){
    						...
    						if(k!=dragO.field){
    							$.ajax({
    								type:'post',
    								url : ctx + "/scheduling/gantt/checkJmSemaState",
    								success:function(result){
    									if(result == "success"){
    										if(yItem.allow){
    											var tempO = dragO;
    											if(tempO.field.indexOf("DP")!=-1){
    			    								if(yItem.id.indexOf("QX")!=-1){
    			    									layer.confirm('确定取消任务吗？', {icon: 3, title:'提示'}, function(index){
    			    										$.ajax({
    			        										type:'post',
    			        										url:ctx+"/scheduling/gantt/cancleTask",
    			        										data:{
    			        											id:tempO.id
    			        										},
    			        										success:function(data){
    			        											if(data=="success"){
    			        												layer.msg("任务已取消！",{icon:1,time:600},function(){
    			        													loadData();
    			        												});
    			        											} else {
    			    									...
    			    									layer.confirm('确认分配任务吗？', {icon: 3, title:'提示'}, function(index){
    			    										$.ajax({
        			    										...
        			    										data:{
        			    											targetActor: yItem.id.replace(/[^0-9]/ig,""),
        			    											jobTaskId : tempO.id,
        			    											procId: tempO.proc
        			    										},
        			    										success:function(data){
    			    								...
    			    								if(yItem.id.indexOf("QX")!=-1){
    			    									layer.confirm('确认终止任务吗？', {icon: 3, title:'提示'}, function(index){
    			    										$.ajax({
        			    										type:'post',
        			    										...
        			    										data:{
        			    											jobTaskId:tempO.id,
        			    											procId:tempO.proc,
        			    											orderId:tempO.order_id
        			    										},
        			    										success:function(data){
        			    											if(data=="succeed"){
        			    												layer.msg("任务已终止！",{icon:1,time:600},function(){
        			    													loadData();
        			    												});
        			    											} else {
        			    												layer.msg("终止失败",{icon:7});
        			    											}
        			    											recoverJmSemaState();
        			    										}
        			    									});
    			    									...
    			    									layer.confirm('确认转交任务吗？', {icon: 3, title:'提示'}, function(index){
    			    										$.ajax({
        			    										type : 'post',
    	...
    	canvas.addEventListener('dblclick',function(e){
    		if(mouse.x>settings.yAxis.width+50 && mouse.x<canvas.width-50 && mouse.y>0 && mouse.y<settings.xAxis.height){
    			...
    			for(var f in ypool){
    				if(isBusy && (ypool[f].id.indexOf("DP")!=-1 || ypool[f].id.indexOf("QX")!=-1)){
    					if(mouse.x>ypool[f].x && mouse.x<ypool[f].x+2*oriYWidth && mouse.y>ypool[f].y && mouse.y<(ypool[f].y+ypool[f].h)){
            				if(ypool[f].expand){
    				...
    				}else if(ypool[f].level == "2" || ypool[f].id.indexOf("DP")!=-1 || ypool[f].id.indexOf("QX")!=-1){
    ...
    var recoverJmSemaState = function(){
    	dragO = null;
    	$.ajax({
			type:'post',
    ...
    var drawGantt = function(){
    	...
    	for(var f in pool){
    		if(ypool[f]){
    			if(ypool[f].y<canvas.height && (ypool[f].y+ypool[f].h)>0){
        			for(var o in pool[f]){
        				if(pool[f][o].start){
        					pool[f][o].x = time2Coor(pool[f][o].start)|0;
        				}else if(pool[f][o].estart){
        					pool[f][o].x = time2Coor(pool[f][o].estart)|0;
        				}
        				if(pool[f][o].end){
        					pool[f][o].x2 = time2Coor(pool[f][o].end)|0;
        				}else if(pool[f][o].eend){
        					pool[f][o].x2 = time2Coor(pool[f][o].eend)|0;
        				}
        				//计划结束时间还未完成时，结束时间拉到当前时间
        				if(pool[f][o].start && !pool[f][o].end && pool[f][o].field != "DP" && pool[f][o].field != "QX"){
    						if(pool[f][o].x2<Math.floor(center)){
    							pool[f][o].x2 = center;
    							pool[f][o].w = pool[f][o].x2 - pool[f][o].x;
    						}
    					}
            			...
            			if(pool[f][o].x<canvas.width && pool[f][o].x2>0 && pool[f][o].y<canvas.height && (pool[f][o].y+pool[f][o].h)>0 && openOverlay != pool[f][o]){
            				if(pool[f][o].x < pool[f][o].x2){
            					...
            					if(pool[f][o].field.indexOf("DP")==-1 && pool[f][o].field.indexOf("QX")==-1){
            						for(var k in pool[f]){
            							if(pool[f][o]!=pool[f][k] && pool[f][o].x<pool[f][k].x2-1 && pool[f][k].x<pool[f][o].x2-1){
            								pool[f][o].clash = true;
            								pool[f][o].color = colorPool.yellow;
            								pool[f][o].h = 0.8*settings.grid.lineHeight|0;
            								pool[f][k].clash = true;
            								pool[f][k].color = colorPool.yellow;
            								pool[f][k].h = 0.8*settings.grid.lineHeight|0;
            							}
            			    		}
            					}
            					if(dragO != pool[f][o]){
            						drawOverlay(pool[f][o].x,pool[f][o].y,pool[f][o].w,pool[f][o].h,color,pool[f][o].text,pool[f][o].icon,pool[f][o].status,false);
            					}else{
            						drawOverlay(pool[f][o].x,pool[f][o].y,pool[f][o].w,pool[f][o].h,color,pool[f][o].text,pool[f][o].icon,pool[f][o].status,true);
            					}
            				}
            			}
            		}
    	...
    	if(openOverlay){
			...
			cxt.shadowBlur = 5;
			drawOverlay(openOverlay.x,openOverlay.y,openOverlay.w,openOverlay.h*1.1,color,openOverlay.text,openOverlay.icon,openOverlay.status,false);
			cxt.shadowBlur = 0;
		}
    	if(dragO){
			drawOverlay(dragO.x,mouse.y-settings.grid.lineHeight/2,dragO.w,dragO.h,dragO.color,dragO.text,dragO.icon,dragO.status,false);
    	...
    	if(types.length>1){
    		...
    		cxt.lineWidth = 3;
    		cxt.setLineDash([]);
    		var tx = (settings.yAxis.width/2)|0;
    		var ty = (settings.xAxis.height/2)|0
    		cxt.beginPath();
        	cxt.moveTo(tx-10,ty-3);
        	cxt.lineTo(tx+8,ty-3);
        	cxt.lineTo(tx+6,ty-5);
        	cxt.moveTo(tx+10,ty+3);
        	cxt.lineTo(tx-8,ty+3);
        	cxt.lineTo(tx-6,ty+5);
        	cxt.stroke();
    	...
    	if(dragingPeople){
			cxt.fillStyle = 'rgba(40, 108, 183)';
			cxt.globalCompositeOperation = "source-over";
	    	cxt.fillRect(dragY.x,mouse.y-dragYcY,oriYWidth,dragY.h);
	    	var item = dragY;
	    	var start = isBusy?item.level:item.level-1;
    ...
    //画覆盖物
	var drawOverlay = function(x,y,w,h,color,text,icon,status,isOdrag){
		if(isOdrag){
			cxt.globalAlpha = 0.3;
		}else{
			cxt.globalAlpha = 1;
		...
		cxt.fillStyle = status=="3"?"#EA2000":color.color;
		...
		if(w2>length+0.7*h+4){
			...
			if(texts.length == 3){
				cxt.fillText(texts[0],x+0.7*h+2,y+h/2);
				var width0 = cxt.measureText(texts[0]).width;
				cxt.font = "bolder "+ Math.ceil(0.4*h)+"px sans-serif";
				cxt.fillText(texts[1],x+0.7*h+2+width0,y+h/2);
				var width1 = cxt.measureText(texts[0]).width + cxt.measureText(texts[1]).width;
				cxt.font = Math.ceil(0.4*h)+"px sans-serif";
				cxt.fillText(texts[2],x+0.7*h+2+width1,y+h/2);
			}else{
	...
	var drawYAxis = function(type,code){
    	...
    	cxt.textBaseline = 'middle';
    	for(var f in ypool){
    		if((!isBusy && ypool[f].level == "1" && ypool[f].id.indexOf("DP") == -1 && ypool[f].id.indexOf("QX") == -1) || (ypool[f].isLeaf && ypool[f].level == "1" && ypool[f].id.indexOf("DP") == -1 && ypool[f].id.indexOf("QX") == -1)){
    			continue;
    		}
    		var item = ypool[f];
    		var start = 1;
    		if(isBusy){
    			start = item.level*1+1;
    		}else{
    			start = item.level=="0"?1:2;
    		}
    		if(ypool[f].id.indexOf("DP") != -1 || ypool[f].id.indexOf("QX") != -1){
    			start = 2;
    		}
			...
			if(!isBusy && item.level == "0" || (isBusy && item.level != "2")){
				cxt.beginPath();
    			cxt.fillStyle = settings.yAxis.color;
    			cxt.setLineDash([]);
    	    	cxt.lineWidth = 0.1;
			...
			if(dragO && (item.level=="2" || item.id.indexOf("DP")!=-1 || item.id.indexOf("QX")!=-1)){
				if(item.allow){
    ...
    function drawDetail(){
    	if(openOverlay){
			...
			if(ox+dw+82<canvas.width){
				...
				cxt.shadowBlur = 0;
				for(var i=0;i<content.length;i++){
					if(i==0 && content[i].indexOf("#")!=-1){
						var conts = content[i].split("#");
						if(conts.length == 3){
							cxt.fillText(conts[0],dx+40,dy+15+(settings.detail.fontSize+5)*(i+1));
							var width0 = cxt.measureText(conts[0]).width;
							cxt.font = "bolder "+(settings.detail.fontSize+1)+"px sans-serif";
							cxt.fillText(conts[1],dx+40+width0,dy+15+(settings.detail.fontSize+5)*(i+1));
							var width1 = cxt.measureText(conts[1]).width;
							cxt.font = settings.detail.fontSize+"px sans-serif";
							cxt.fillText(conts[2],dx+40+width0+width1,dy+15+(settings.detail.fontSize+5)*(i+1));
						}else{
