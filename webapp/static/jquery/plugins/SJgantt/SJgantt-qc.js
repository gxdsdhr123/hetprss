(function ($) {
	...
	function handleYData(result){
    	...
    	for(var i=0;i<result.length;i++){
        	...
        	ypool[f.id] = f;
        	if(!isBusy && f.level == "2" && f.pid != "所有航空公司" && f.pid.indexOf("未分配") == -1){
        		isBusy = true;
	...
	function calcY(){
		...
		ypool["JWQCSBZqsc"].y = tempY;
		ypool["JWQCSBZqsc"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCSBZqsc",tempY);
		ypool["JWQCSBZqsc"].h = tempY-ypool["JWQCSBZqsc"].y; 
		ypool["JWQCSBZwsc"].y = tempY;
		ypool["JWQCSBZwsc"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCSBZwsc",tempY);
		ypool["JWQCSBZwsc"].h = tempY-ypool["JWQCSBZwsc"].y; 
		ypool["JWQCSBZljc"].y = tempY;
		ypool["JWQCSBZljc"].h = settings.grid.lineHeight;
		tempY = calcCustomY("JWQCSBZljc",tempY);
		ypool["JWQCSBZljc"].h = tempY-ypool["JWQCSBZljc"].y; 
