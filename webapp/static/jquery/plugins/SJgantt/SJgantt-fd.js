(function ($) {
	...
	function handleYData(result){
        ...
        for(var i in ypool){
        	if(ypool[ypool[i].pid]){
        		ypool[ypool[i].pid].isLeaf = false;
        		ypool[ypool[i].pid].pre = "+";
        	}
        }
	}
	var field = function(f){
		...
		this.show = this.level==settings.yData.startLevel?true:false;
		this.isLeaf = true;
		this.expand = false;
	}
    //加载数据
	...
	function handleData(result){
		...
		pool = {};
		for(var i in result){
			if(ypool[result[i].field]){
				ypool[result[i].field].size++;
				if(ypool[result[i].field].level>1){
					ypool[ypool[result[i].field].pid].size++;
				}
			}
		}
		for(var j in result){
	...
	var overlay = function(d){
		...
		this.y = ypool[this.field]?ypool[this.field].y:0;
        ...
        this.cacheCxt = this.cacheCanvas.getContext("2d");
        this.cacheCanvas.width = this.w;
        this.cacheCanvas.height = settings.grid.lineHeight;
        this.validate();
	}
	overlay.prototype = {
		validate:function(){
	...
	function calcY(){
		...
		for(var f in ypool){
			if(ypool[f].level == "1"){
				if(!settings.showEmptyRow && ypool[f].size == 0){
					ypool[f].show = false;
					continue;
				}
				ypool[f].y = tempY;
				if(ypool[f].open){
					for(var s in ypool){
						if(ypool[s].pid == ypool[f].id){
							if(!settings.showEmptyRow && ypool[s].size == 0){
								ypool[s].show = false;
								continue;
							}
							ypool[s].y = tempY;
							...
							if(ypool[s].expand){
								ypool[s].list = [];
								for(var o in pool[s]){
									if(ypool[s].list.length==0){
										ypool[s].list.push([[pool[s][o].x,pool[s][o].x2]]);
										pool[s][o].y = ypool[s].y;
									}else{
										var rowClash = true;
										loop:
										for(var j=0;j<ypool[s].list.length;j++){
											var colClash = false;
											for(var k=0;k<ypool[s].list[j].length;k++){
												if(pool[s][o].x<ypool[s].list[j][k][1] && ypool[s].list[j][k][0]<pool[s][o].x2){
													colClash = true;
													break;
												}
											}
											if(!colClash){
												ypool[s].list[j].push([pool[s][o].x,pool[s][o].x2]);
												pool[s][o].y = ypool[s].y + j*settings.grid.lineHeight;
												rowClash = false;
												break;
											}
										}
										if(rowClash){
											ypool[s].list.push([[pool[s][o].x,pool[s][o].x2]]);
											ypool[s].h += settings.grid.lineHeight;
											pool[s][o].y = ypool[s].y+(ypool[s].list.length-1)*settings.grid.lineHeight;
										}
									}
								}
							}else{
        			...
        			if(ypool[f].show){
            			if(mouse.x>ypool[f].x && mouse.x<ypool[f].x+oriYWidth && mouse.y>ypool[f].y && mouse.y<ypool[f].y+ypool[f].h){
            				if(ypool[f].open){
            					...
            					if(getOpenNum()==0){
            						settings.yAxis.width = ypool[f].level*1*oriYWidth;
            					}
            				}else{
            					if(!ypool[f].isLeaf){
            						settings.yAxis.width = (ypool[f].level*1+1)*oriYWidth;
            						ypool[f].open = true;
            						calcY();
            					}
            				}
            	    		...
            	    		if(mouse.x>o.x && mouse.x<o.x2 && mouse.y>o.y && mouse.y<(o.y+o.h)){
            	    			if(openOverlay == o){
            		    			...
            		    			if(mouse.x < (o.x+o.x2)/2){
                	    				window.clickRowId = (o.infltid || o.outfltid);
                	    			}else{
                	    				window.clickRowId = (o.outfltid || o.infltid);
                	    			}
            		    			window.clickRow = o;
            	    			}
            	    			return false;
    ...
    var drawGantt = function(){
    	...
    	for(var f in pool){
    		if(ypool[f]){
    			if(ypool[f].show && ypool[f].y<canvas.height && (ypool[f].y+ypool[f].h)>0){
        			for(var o in pool[f]){
        				pool[f][o].x = time2Coor(pool[f][o].start);
        				pool[f][o].x2 = time2Coor(pool[f][o].end);
        				pool[f][o].w = pool[f][o].x2 - pool[f][o].x;
            			if(pool[f][o].x<canvas.width && pool[f][o].x2>0 && pool[f][o].y<canvas.height && (pool[f][o].y+pool[f][o].h)>0 && openOverlay != pool){
            				if(pool[f][o].x < pool[f][o].x2){
            					var color = ypool[f].expand?pool[f][o].ocolor:pool[f][o].color;
            					drawOverlay(pool[f][o].x,pool[f][o].y,pool[f][o].w,pool[f][o].h,color,pool[f][o].text)
            				}
            			}
            		}
        		}
    	...
    	if(openOverlay){
			...
			cxt.shadowBlur = 5;
			drawOverlay(openOverlay.x,openOverlay.y,openOverlay.w,openOverlay.h*1.1,color,openOverlay.text);
			cxt.shadowBlur = 0;
		}
    	//X轴
    	drawXAxis();
    ...
    //画覆盖物
	var drawOverlay = function(x,y,w,h,color,text){
	...
	var drawYAxis = function(type,code){
    	...
    	for(var f in ypool){
    		...
    		if(f.show){
    			cxt.fillText(f.pre+" "+f.name,f.x+oriYWidth/2,(f.y+settings.grid.lineHeight/2));
    			if(f.level == "1"){
    				cxt.beginPath();
        			cxt.fillStyle = settings.yAxis.color;
        			cxt.setLineDash([]);
