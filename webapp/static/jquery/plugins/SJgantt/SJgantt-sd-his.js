(function ($) {
	...
	function loadData(){
		...
		$.ajax({
    		...
    		success:function(result){
    			pool = [];
    			hisDate = settings.queryParams.hisDate;
        			...
        			if(pool[f] && mouse.y>ypool[f].y && mouse.y<ypool[f].y+ypool[f].h){
        				for(var i=pool[f].length-1;i>=0;i--){
            	    		...
            	    		if(mouse.x>o.x && mouse.x<o.x2 && mouse.y>o.y && mouse.y<o.y+o.h){
            	    			if(o.field == "QX" || o.color==SJgantt.GREEN || o.color==SJgantt.BLUE){
            	    				return false;
            	    			}
    		...
    		/*if(dragO){
    			for(var k in ypool){
    				if(ypool[k].level == "2" || k == "DP" || k == "QX"){
    					var dragOY = Math.floor(mouse.y-dragCy+dragO.h/2);
    						...
    						if(k!=dragO.field){
    							if(dragO.field == "DP"){
    								if(k == "QX"){
    									...
    									layer.confirm('确认分配任务吗？', {icon: 3, title:'提示'}, function(index,dragO){
    										dragO.field = k;
  										  layer.close(index);
  										});
    									$.ajax({
    ...
    var drawGantt = function(){
    		...
    		if(ypool[f]){
    			if(ypool[f].y<canvas.height && (ypool[f].y+ypool[f].h)>0){
        			for(var o in pool[f]){
        				...
        				/*if(!pool[f][o].end && pool[f][o].icon != "未" && pool[f][o].icon != "完"){
            			...
            			if(pool[f][o].x<canvas.width && pool[f][o].x2>0 && pool[f][o].y<canvas.height && (pool[f][o].y+pool[f][o].h)>0 && openOverlay != pool[f][o]){
            				if(pool[f][o].x < pool[f][o].x2){
            					...
            					if(pool[f][o].field.indexOf("DP")==-1 && pool[f][o].field.indexOf("QX")==-1){
            						for(var k in pool[f]){
            							if(pool[f][o]!=pool[f][k] && pool[f][o].x<pool[f][k].x2 && pool[f][k].x<pool[f][o].x2){
            								pool[f][o].clash = true;
            					...
            					drawOverlay(pool[f][o].x,pool[f][o].y,pool[f][o].w,pool[f][o].h,color,pool[f][o].text,pool[f][o].icon,pool[f][o].status);
            				}
            			}
            		}
        		}
    	...
    	if(openOverlay){
			...
			cxt.shadowBlur = 5;
			drawOverlay(openOverlay.x,openOverlay.y,openOverlay.w,openOverlay.h*1.1,color,openOverlay.text,openOverlay.icon,openOverlay.status);
			cxt.shadowBlur = 0;
		}
    	if(dragO){
			drawOverlay(dragO.x,mouse.y-dragCy,dragO.w,dragO.h,dragO.color,dragO.text,dragO.icon,dragO.status);
		}
    	//X轴
    	drawXAxis();
    	//当前时间线
    ...
    //画覆盖物
	var drawOverlay = function(x,y,w,h,color,text,icon,status){
		...
		cxt.fillStyle = status=="3"?colorPool.red.back:color.color;
