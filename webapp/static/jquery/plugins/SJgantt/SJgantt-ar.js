(function ($) {
	...
	var defaultOptions = {
            ...
            background:"url(../../../../images/SJgantt-bg.jpg) no-repeat scroll 0% 0% / 100% 100%",
	...
	var overlay = function(d){
		...
		this.color = colorPool.blue;
		this.ocolor = colorPool.blue;
		this.text = d.text;
    ...
    var addEvent = function(){
    	...
    	canvas.addEventListener('mouseup',function(e){
    		...
    		if(dragO){
    			for(var k in ypool){
					if(mouse.y>ypool[k].y && mouse.y<ypool[k].y+ypool[k].h){
						for(var o in pool[dragO.field]){
							if(pool[dragO.field][o] == dragO){
								pool[dragO.field].splice(o,1);
								dragO.field = k;
								dragO.y = ypool[k].y;
								if(!pool[k]){
									pool[k] = [];
    ...
    var drawGantt = function(){
            				...
            				if(pool[f][o].x < pool[f][o].x2){
            					...
            					if(f != "DP"){
            						...
            						if(!hasClash){
            							...
            							pool[f][o].color = pool[f][o].ocolor;
            						}
            					}
    	...
    	if(dragO){
			drawOverlay(dragO.x,mouse.y-dragO.h/2,dragO.w,dragO.h,dragO.color,dragO.text);
		}
    	//X轴
    	drawXAxis();
    	//当前时间线
    	...
    	if(mouse.x>settings.yAxis.width){
    		if(contentHeight>canvas.height-settings.xAxis.height){
    			if(mouse.x<canvas.width-10){
            		...
            		cxt.fillRect(mouse.x+15,mouse.y+15,35,20);
            		cxt.fillStyle = "#ccc";
            		cxt.font = "10px sans-serif";
                	cxt.fillText(coor2Time(mouse.x).substring(11),mouse.x+32,mouse.y+30);
    			}
    		}else{
    			drawDashedLine(mouse.x,settings.xAxis.height,mouse.x,canvas.height,[2,2],0.5,'#FFFFFF');
        		cxt.fillStyle = "rgba(0,0,0,0.5)";
	...
	var drawOverlay = function(x,y,w,h,color,text){
		...
		if(w2>length+4){
			...
			cxt.fillText(text,x+ix+2,y+h/2);
		}
		cxt.beginPath();
    ...
    function drawDetail(){
    	if(openOverlay){
	    	...
	    	var color = ypool[openOverlay.field].expand?openOverlay.ocolor:openOverlay.color;
	    	var ox = o.x2;
			var oy = o.y+Math.ceil((settings.grid.lineHeight-o.h)/2)+0.4*(o.h);
