(function(snaker){
	...
	designer.config={
		...
		rect:{
			...
			text:{text:"状态","font-size":13,fill:"#fff"},
			margin:5,
			props:[],
			img:{}
		},
		path:{
			...
			textPos:{x:0,y:-10},
			props:{
				name:{name:"name",label:"名称",value:""},
				displayName:{name:"displayName",label:"显示",value:""},
                expr:{name:"expr",label:"表达式",value:""}
			}
		},
		tools:{
			attr:{
	...
	designer.rect=function(o, r){
      ...
      //baochl_20171206
        _text.click(function(){
        	if(_o.props.nodeId&&_o.props.nodeId.value){
        		showNodeInfo(_o.props.nodeId.value);
        	}
        });
	...
	designer.init=function(c, o){
		...
		if(designer.config.editable){
			...
			snaker(c).droppable({
				...
				drop:function(c,i){
					var states = {
							attr:{x:i.helper.offset().left,y:i.helper.offset().top}
						};
						if(i.helper.attr("type")!="start"&&i.helper.attr("type")!="end"){
							states["text"] = {text:i.helper.attr("label")};
							states["props"] = {
									  nodeId:{value:i.helper.attr("nodeId")},
								      name:{value:i.helper.attr("name")},
									  displayName:{value:i.helper.attr("label")},
								      jobKind:{value:i.helper.attr("jobKind")},
								      jobType:{value:i.helper.attr("jobType")},
								      aftMsg1:{value:i.helper.attr("aftMsg1")},
								      aftMsg2:{value:i.helper.attr("aftMsg2")},
								      notifyMsg:{value:i.helper.attr("notifyMsg")},
								      alarmMsgLv1:{value:i.helper.attr("alarmMsgLv1")},
								      alarmMsgLv2:{value:i.helper.attr("alarmMsgLv2")},
								      alarmMsgLv3:{value:i.helper.attr("alarmMsgLv3")},
								      alarmMsgLv4:{value:i.helper.attr("alarmMsgLv4")},
								      alarmMsgLv5:{value:i.helper.attr("alarmMsgLv5")},
								      notifyTm:{value:i.helper.attr("notifyTm")},
								      alarmTm1:{value:i.helper.attr("alarmTm1")},
								      alarmTm2:{value:i.helper.attr("alarmTm2")},
								      alarmTm3:{value:i.helper.attr("alarmTm3")},
								      alarmTm4:{value:i.helper.attr("alarmTm4")},
								      alarmTm5:{value:i.helper.attr("alarmTm5")},
								      icon:{value:i.helper.attr("icon")}
								     }
						}
					snaker(_r).trigger("addrect",[i.helper.attr("type"),states])
				}
			});
			snaker("#save").click(function(){
                ...
                if(!_states||$.isEmptyObject(_states)){
                	alert("流程定义不能为空！");
                	return false;
                }
                if(!_paths||$.isEmptyObject(_paths)){
                	alert("请设置连接线！");
                	return false;
                }
                var nodeCount = 0;//节点个数
                var pathCount = 0;//连接线个数
                var startCount = 0;//开始节点个数
                var endCount = 0;//结束节点个数
				for(var node in _states){
					if(_states[node]){
						nodeCount++;
						var nodeXml = _states[node].toBeforeXml();
						if(nodeXml&&$.trim(nodeXml).indexOf("\<start ")>=0){
							startCount++;
						}
						if(nodeXml&&$.trim(nodeXml).indexOf("\<end ")>=0){
							endCount++;
						}
						i+=nodeXml;
