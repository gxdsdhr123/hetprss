...
layui.use(['layer','form'],function(){
	...
	form.on('radio(dayNight)',function(data){
		var val = data.elem.value;
		initParam.dayOrNight = val;
		refresh();
	});
	...
	form.on('select(yOrN)',function(data){
		var y_or_n = data.elem.value;
		if(y_or_n == 'ALL'){
			y_or_n = '';
		}
		initParam.yOrN = y_or_n;
		refresh();
	});
	
	form.on('select(inOrOut)',function(data){
		var in_or_out = data.elem.value;
		if(in_or_out == 'ALL'){
			in_or_out = '';
		}
		initParam.inOrOut = in_or_out;
		refresh();
	});
...
//			if( targettype == 'YP' || targettype == 'NP'){
			...
			function runSetTask(){
				...
				if(msg == 'success'){
					...
					if(target.find('.drag-item').length < 5){
						// 人员新增任务
						var targetDragItem = $('#monitor').TaskChart('createDraggable',data.flightName,data,'NP');
						if(if_high){
					...
					// 航班任务变为已分配的人
					dragFrom.html(data.taskName+'</br>'+target.attr('search-text') + '('+data.taskStatus+')');
					dragFrom.removeClass();
					dragFrom.addClass('drag-item');
...
//			if(targettype == 'YP' || targettype == 'NP'){
			...
			function runSetTask(){
				...
				if(msg == 'success'||msg=='succeed'){
					...
					var if_high;
					// 删除原任务人员
					var oldItem = $('.airport-item .drag-item[task='+data.id+']');
					if(oldItem.hasClass('high')){//高亮
						if_high = true;
					}
					...
					if(target.find('.drag-item').length < 5){
						// 人员新增任务
						var targetDragItem = $('#monitor').TaskChart('createDraggable',data.flightName,data);
						if(if_high){
							targetDragItem.addClass('high');
						}
						if(targettype == 'YP'){
...
//		$('#terminal').addClass('terminal-dragging');
	...
	// 拖拽人员到机坪，进行人员分工修改
	/*$('#monitor').on('person.title.mousedown',function(e,thisObj,data,type){
		// 清除
		try{
			dragItem.remove();
			dragItem = null;
...
//				$('#terminal').removeClass('terminal-dragging');
	...
	$('#monitor').on('drag.rclick',function(e,event,thisObj,data,type){
        ...
        if(offsetX > $(window).width() - 160){
    	 ...
    	 if(event.clientY > $(window).height() - 50){
    		 offsetY -= 50;
						...
						$.ajax({
					        ...
					        success: function(msg){
					            if(msg == 'success'){
					            	// 任务块状态变更
									data.jobState = 6;
									var title = data.taskName;
									$(thisObj).html(title);
									$(thisObj).removeClass().addClass('drag-item').addClass('item-jobState-6');
									/** 重新定义事件*/
									...
									// 删除人员中对应的任务块
									var personItem = $('.airport-item .drag-item[task='+data.id+']');
...
// 			 						data.status = 0;
 			 						data.jobState = 0;
 			 						$(thisObj).removeClass('item-jobState-6').addClass('item-jobState-0');
 					            }else{
 					            	layer.msg(msg,{icon:7});
 					            }
 					         }
	...
	$('#monitor').on('person.title.rclick',function(e,event,thisObj,data,type){
        ...
        var offsetY = event.clientY - toolbarHeight;
    	if(event.clientY > $(window).height() - 170){
    		offsetY -= 170;
    		if(offsetY <20){offsetY=20}
        }*/
    	 ...
    	 var popArr = [{
			...
			callback : function(e) {
				...
				var set = layer.open({
					type : 2,
					title : "车辆定位",
					content : carHostUrl + 'page/toRealMonitor?vehicleIds=' + data.carId,
					btn : [ "关闭" ],
			...
			callback : function(e) {
				layer.open({
					...
					yes:function(index, layero){
						var start = layero.find(".rangeDate").eq(0).val();
						var end = layero.find(".rangeDate").eq(1).val();
						if(!start||!end){
							layer.msg('请选择车辆轨迹的开始时间或结束时间', {
								icon : 2
							});
							return false;
						}else{
							...
							var set = layer.open({
								type : 2,
								title : "车辆轨迹",
								content : encodeURI(carHostUrl + 'page/toHistory?vehicleIds=' + data.carId +
...
//    	 $.ajax({
//				type: "POST",
//				url: ctx + '/taskmonitor/selectFenGong',
//				data: {schemaId : $('#schemaId').val(),operator:data.personId},
//				dataType: "json",
//				async:false,
//				success: function(dataFenGong){
////					$.each(dataFenGong.data,function(k,v){
//						var childrenJiWei = [];
//						$.ajax({
//							type: "POST",
//							url: ctx + '/taskmonitor/selectJiWei',
//							data: {
//								schemaId : $('#schemaId').val(),
//								operator:data.personId,
//								fengongId:dataFenGong.data[0].ID
//							},
//							dataType: "json",
//							async:false,
//							success: function(dataJiWei){
//								$.each(dataJiWei.data,function(m,n){
//									childrenFenGong.push({
//										name:n.AREA_NAME,
//										callback : function(e) {
//											saveFengongJiwei(dataFenGong.data[0].ID,n.ID,data.personId);
//										}, 
//										
//									});
//								})
...
// 	 	});
    	...
    	// 加载二级菜单项
     	...
     	$.each(jobTypes,function(i,item){
     		...
     		$.ajax({
				...
				data: {schemaId : $('#schemaId').val(),jobType:typeCode},
				dataType: "json",
				async : false,
				success: function(dataMap){
					if(dataMap.code == 0){
	...
	$('#monitor').on('flight.title.mouseover',function(e,event,thisObj,data,type){
		...
		popTimer = setTimeout(function(){
			...
			var pop = $('#flight');
			pop.width(500).height(150);
			$.ajax({
				type: "POST",
				url: ctx + '/taskmonitor/getFlightInfo',
				data: {schemaId : $('#schemaId').val(),fltid:data.id,type:data.inOutFlag,updateSession:0},
				dataType: "json",
				success: function(dataMap){
					if(dataMap.code == '0'){
...
// 右键修改人员分工界面
function showWorkerDivision(operator){
	layer.open({
		type : 2,
		title : false,
		closeBtn : false,
		area : [ '90%', '60%' ],
		btn : [ "返回" ],
		content : [ ctx + "/division/info/list?operator="+operator ]
	});
}

//右键修改人员分工界面 
function saveFengongJiwei(fengongId,jiweiId,operator){
	$.ajax({
		type: "POST",
		url: ctx + '/taskmonitor/saveFengongJiwei',
		data: {
			schemaId : $('#schemaId').val(),
			fengongId:fengongId,
			jiweiId:jiweiId,
...
function showLegend(){
	...
	layer.open({
		type:2,
		title:"图例说明",
		content:ctx + '/taskmonitor/legend?schemaId=7',
		area:['1100px','600px'],
		btn:['关闭']
	});
}
