	        ...
	        success: function(resultMap){
	        	if(resultMap.code == 0){
	        		...
	        		$.each(resultMap.data,function(i,personId){
	        			// 最优推荐样式
	        			var personContainer = $('#airport .item-container[data-id='+personId+'],#terminal .item-container[data-id='+personId+']');
	        			if(personContainer.length > 0 && tuijian <= 3){
	        				personContainer.addClass('tuijian-' + tuijian);
	        				tuijian++;
	        			}
	        			$('#airport .item-container[data-id='+personId+'],#terminal .item-container[data-id='+personId+']').addClass('dragging');
	        		});
...
//			if(targettype == 'YP' || targettype == 'NP'){
			...
			function runSetTask(resultData){
				...
				if(msg == 'success'||msg=='succeed'){
					...
					var if_high;
					// 删除原任务人员
					var oldItem = $('#airport .drag-item[task='+data.id+'],#terminal .drag-item[task='+data.id+']');
						...
						$.ajax({
					        ...
					        success: function(msg){
					            if(msg == 'success'){
									...
									data.jobState = 6;
									var title;
									if(data.jobType == 'JWBDCcsc'){
										title = '残升车';
									}else if(data.jobType == 'JWBDCkst'){
										title = '考斯特';
									}else{
										title = '&nbsp;';
									}
	...
	$('#monitor').on('flight.title.mouseover',function(e,event,thisObj,data,type){
		...
		popTimer = setTimeout(function(){
			...
			$.ajax({
				...
				data: {schemaId : $('#schemaId').val(),fltid:fltid,type:data.inOutFlag,updateSession:0},
				dataType: "json",
				success: function(dataMap){
					if(dataMap.code == '0'){
...
function showLegend(){
	...
	layer.open({
		type:2,
		title:"图例说明",
		content:ctx + '/taskmonitor/legend?schemaId=102',
		area:['1100px','600px'],
		btn:['关闭']
	});
}
