...
layui.use(['layer','form'],function(){
	...
	// 禁用右键菜单
	$('body').off("mousedown").on("contextmenu", function (e) {
        e.preventDefault();
        return false;
    });
...
//					if(target.find('.drag-item').length < 5){
						...
						if(targettype == 'YP'){
							target.find('.people-task-body').prepend(targetDragItem);
						}
//						else{
...
//					}
					// 航班任务变为已分配的人
					dragFrom.html(target.attr('search-text') + '('+data.taskStatus+')');
...
//				$('#terminal').removeClass('terminal-dragging');
	...
	$('#monitor').on('drag.rclick',function(e,event,thisObj,data,type){
						...
						$.ajax({
					        ...
					        success: function(msg){
					            if(msg == 'success'){
									...
									var title = '&nbsp;';
									$(thisObj).html(title);
									$(thisObj).removeClass('item-status-1').removeClass('item-status-2').addClass('item-status-6');
									/** 重新定义事件*/
    	...
    	// 人员任务新增   /释放人员    /任务重置
    	 if($(thisObj).parents('.airport-item').length > 0){
    		 if(data.status == 1 || data.status == 2){
    			 // 释放人员
    			 rightArr.push({
    				name : '释放人员',
						...
						$.ajax({
					        ...
					        success: function(msg){
					            if(msg == 'success'){
									...
									data.status = 6;
									data.jobState = 6;
									var flightItem = $('.nFlight,.yFlight .drag-item[task='+data.id+']');
									flightItem.html("&nbsp");
									flightItem.removeClass('item-status-1').removeClass('item-status-2').addClass('item-status-6');
									/** 重新定义事件*/
									flightItem.off('click').off('mousedown');
									flightItem.on("mousedown", function (event) {
							            if (event.which == 3) {
							            	// 绑定右键事件
							            	$this.trigger('drag.rclick',[event,this,data,type]);
							            }else{
							...
							success: function(dataJiWei){
								$.each(dataJiWei.data,function(m,n){
									childrenJiWei.push({
										name:n.AREA_NAME,
										callback : function(e) {
											saveFengongJiwei(v.ID,n.ID,data.personId);
										}, 
    	...
    	// 加载二级菜单项
     	...
     	$.each(jobTypes,function(i,item){
     		...
     		$.ajax({
				...
				data: {schemaId : $('#schemaId').val(),jobType:typeCode,inOut:data.inOutFlag},
				dataType: "json",
				async : false,
				success: function(dataMap){
					if(dataMap.code == 0){
				...
				success: function(dataMap){
					if(dataMap.code == '0'){
						pop.find('span').each(function(){
							...
							if((k == 'inFltNum' || k == 'outFltNum') && !v){
								v = '----';
							}else if((k == 'inVipFlag' || k == 'outVipFlag') && !v){
								v = '--';
							}
							setProperty(pop,k,v);
						});
...
function showLegend(){
	...
	layer.open({
		type:2,
		title:"图例说明",
		content:ctx + '/taskmonitor/legend?schemaId=5',
		area:['1100px','600px'],
		btn:['关闭']
	});
}
