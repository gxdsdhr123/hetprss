...
function occupyTable(data){
	...
	$.each(data,function(i,item){
		...
		tr.append('<td>' + (item.NAME ? item.NAME : '&nbsp;' )+ '</td>');
		tr.append('<td>' + (item.STATE ? item.STATE : '&nbsp;') + '</td>');
		tr.append('<td>' + (item.FLIGHT_NUM ? item.FLIGHT_NUM : '0') + '</td>');
		tr.append('<td>' + (item.WORKLOAD	 ? item.WORKLOAD	 : '0') + '</td>');
		tr.append('<td>' + (item.BLOCKUP ? item.BLOCKUP : '&nbsp;') + '</td>');
		
		$('#occupyTable tbody').append(tr);
	});
	
	if(!limarquee){
		limarquee = $('#occupyTableBody').liMarquee({
			direction: 'up',
			runshort: false
		});
	}else{
