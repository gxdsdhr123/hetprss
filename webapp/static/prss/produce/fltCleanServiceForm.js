...
$(function() {
	layui.use([ 'layer', 'form' ], function() {
		...
		if(operateType=="modify"){
			...
			$("#departAptCode").attr("disabled","disabled");
...
function searchFlightDetail(){
	...
	$.ajax({
        type: "POST",
        url: ctx + '/produce/fltCleanService/flightDetail',
        data: {
        ...
        success: function(reFltDetailData){
        	...
        	if(reFltDetailData.result=="found"){
            	...
            	$("#ataReal").val(fltDetailData.ata);
            	$("#ata").val(fltDetailData.ataStr);
            	$("#departAptcodeReal").val(fltDetailData.departAptcode);
            	$("#departAptCode").val(fltDetailData.departAptCnname);
            	form.render();
        	}else{
        		$("#aircraftNumber").val("");
        		$("#acttypeCodeReal").val("");
