...
$(function() {
	...
	$("#down").click(function(){
		...
		$("#exportForm").attr("action",ctx + "/produce/fltChargeService/exportExcel?startDate="+startDate+"&endDate="+endDate);
		$("#exportForm").submit();
	});
	//删除单据
	$("#delete").click(function(){
		if(clickRowId==null || clickRowId==''){
			...
			$.ajax({
		        type: "POST",
		        url: ctx + '/produce/fltChargeService/delete',
		        data: {
		        	"id":clickRowId
		        },
	...
	$('#modify').click(function() {
		if(clickRowId==null || clickRowId==''){
			...
			win = layer.open({
				...
				area:['610px','440px'],
				closeBtn : false,
				content : ctx + "/produce/fltChargeService/toForm?operateType=modify&id="+clickRowId,
				btn : [ "保存", "取消" ],
	...
	$("#add").click(function() {
		win = layer.open({
			...
			area:['610px','440px'],
			closeBtn : false,
			content : ctx + "/produce/fltChargeService/toForm?operateType=add&id=",
			btn : [ "保存", "取消" ],
			yes : function(index, layero) {
				var iframeWin = window[layero.find('iframe')[0]['name']];
				iframeWin.doSave();
	...
	baseTable = $("#baseTable");
	var tableOptions = {
		url: ctx + "/produce/fltChargeService/getData", 
	    method: "get",
	    dataType: "json",
	    align:"center",
		striped: true,
	    ...
	    columns: [
			...
			{field: "addCleanStr", title: "加清",editable:false},
			{field: "sewerageStr", title: "排污",editable:false},
			{field: "operatorName", title: "操作人",editable:false},
			{	
				field : "PICTURE",
				title : '乘务长签字',
				editable:false,
