...
$(function(){
	...
	$.ajax({
		...
		success : function(result) {
			...
			receiverData =(new Function("","return "+result))();
		}
	});
	...
	$("#douxf").blur(function(){
		...
		if($("#flightDate").val()=="" && $("#douxf").val()==""){
			return false;
		}
		if($("#flightDate").val()==null && $("#flightDate").val()==null){
			return false;
		}
...
function save(){
	...
	$.ajax({
		url:ctx+"/arrival/produce/countGulf",
		type : "POST",
		...
		success:function(result){
			...
			if(result==0){
				$.ajax({
					url:ctx+"/arrival/produce/save",
					type : "POST",
					data :{data: data,"flightDate":flightDateI,"flightNumber":flightNumberI,"aircraftNumber":aircraftNumberI,"actstandCode":actstandCodeI,"ata":ataI,"chali":chaliI},
					success:function(result){
						if(result!=0){
				...
				layer.msg("当天的航班号已经存在了", {icon: 0,time: 1000},function(){
					layer.close(index);
					return false;
				});
			}
