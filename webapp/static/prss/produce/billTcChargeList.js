...
$(document).ready(function() {
	...
	$("#newBtn").click(function() {
		...
		set = layer.open({
			...
			area:['1000px','80%'],
			closeBtn : false,
			content : ctx + "/produce/tcCharge/add?type="+type,
			btn : [ "保存", "取消" ],
...
//		layer.full(set);// 展开弹出层直接全屏显示
	...
	$("#editBtn").click(function() {
		...
		set = layer.open({
			...
			area:['1000px','80%'],
			closeBtn : false,
			content : ctx + "/produce/tcCharge/edit?type="+type+"&id="+clickRowId,
			btn : [ "保存", "取消" ],
			btn1 : function(index, layero) {
				var iframeWin = window[layero.find('iframe')[0]['name']];
				iframeWin.save();
	...
	$("#wordBtn").click(function() {
		if(clickRowId==null || clickRowId==''){
			...
			$("#id").val(clickRowId);
			$("#type").val("BillTcChargeServiceTemplet");
			$("#printWord").submit();
		}
	});
...
function initGrid() {
	$("#baseTable").bootstrapTable({
		...
		idField : "id",
		uniqueId : "rowId",
		url : ctx + "/produce/tcCharge/dataList",
		method : "get",
		pagination : false,
		showRefresh : false,
		clickToSelect : false,
