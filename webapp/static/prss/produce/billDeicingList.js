...
$(document).ready(function() {
	...
	$("#newBtn").click(function() {
		...
		set = layer.open({
			...
			area:['1000px','80%'],
			closeBtn : false,
			content : ctx + "/produce/deicing/add?type="+type,
			btn : [ "保存", "取消" ],
...
//		layer.full(set);// 展开弹出层直接全屏显示
	...
	$("#editBtn").click(function() {
		...
		set = layer.open({
			...
			area:['1000px','80%'],
			closeBtn : false,
			content : ctx + "/produce/deicing/edit?type="+type+"&id="+clickRowId,
			btn : [ "保存", "取消" ],
			btn1 : function(index, layero) {
				var iframeWin = window[layero.find('iframe')[0]['name']];
				iframeWin.save();
...
function initGrid() {
	...
	$("#baseTable").bootstrapTable({
		...
		idField : "id",
		uniqueId : "rowId",
		url : ctx + "/produce/deicing/dataList",
		method : "get",
		pagination : false,
		showRefresh : false,
		clickToSelect : false,
