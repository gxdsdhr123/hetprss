		...
		success : function(data) {
			if(data.code == 0){
				...
				$.each($("#selUnfixedUL li"),function(k,v){
					var crslStr = $(v).data("code");
					crslArr.push(crslStr.split(";")[0]);
				})
				paramsJsonObj['crslStr'] = crslArr.join(",");
			}else{
...
function addParam(){
	layer.open({
		type : 2,
		title : '编辑表达式',
		content : ctx + "/stand/carousel/openParam?schemaId=101",
		btn : [ "确定",  "取消" ],
		area : [ '800px', '450px' ],
		yes : function(index, layero) {
			var iframeWin = window[layero.find('iframe')[0]['name']];
			...
			$.ajax({
				type : 'post',
				url : ctx + "/stand/carousel/saveParam?schemaId=101",
				data : {
					data : submitData
				},
				async : true,
