...
$(function() {
	...
	$('#createRule').click(function() {
		iframe = layer.open({
			...
			closeBtn : false,
			area:['100%','100%'],
			content : ctx + "/rule/waterFillRule/form?type=create&id=",
			btn : [ "保存", "取消" ],
			yes : function(index, layero) {
				...
				if(iframeWin.doCheck()){
					$.ajax({
						type : 'post',
						url : ctx + "/rule/waterFillRule/save",
						data : {
							data : submitData
						},
						async : true,
		...
		if(clickRowId==null){
			...
			var id = clickRowId;
				iframe = layer.open({
					...
					area:['100%','100%'],
					closeBtn : false,
					content : ctx + "/rule/waterFillRule/form?type=update&id=" + id,
					btn : [ "保存", "取消" ],
					yes : function(index, layero) {
						var iframeWin = window[layero.find('iframe')[0]['name']];
						var submitData = iframeWin.getSubmitData();
		...
		if(clickRowId==null){
			...
			var id = clickRowId;
				...
				// 按钮
					...
					$.ajax({
						type : 'post',
						url : ctx + "/rule/waterFillRule/delete",
						data : {
							ruleId : id
						},
						async : false,
	...
	baseTable = $("#baseTable");
	var tableOptions = {
		url: ctx + "/rule/waterFillRule/ruleList", 
	    method: "get",
	    dataType: "json",
		striped: true,
	    cache: true,
	    ...
	    columns: [
			...
			{field: "ADDEDWATER", title: "加水量",editable:false,align:'left',halign:'center'},
