...
<script type="text/javascript" src="${ctxStatic}/prss/rule/setup/fdFilterCheck.js"></script>
<script>
	var items = ${items};
</script>
</head>
				...
				<div>
					<ul class="list-group sortable choosedField down" id="selectul"
						...
						<c:forEach items="${sitems}" var="item">
							<li class="list-group-item" id="${item}" onclick="liclick(this);"
								onmousedown="lidown(this);" onmousemove="limove(this);" onmouseup="liup(this);">${item}</li>
						</c:forEach>
					</ul>
				</div>
			</div>
