...
<%@ attribute name="layVerify" type="java.lang.String" required="false" description=""%>
<div class="input-group input-group-sm input-append">
	<input id="${id}Id" name="${name}" class="${cssClass}" type="hidden" value="${value}"/> 
	<input id="${id}Name" 
		   name="${labelName}" ${allowInput?'':'readonly="readonly"'} 
		   type="text" value="${labelValue}" 
		   data-msg-required="${dataMsgRequired}" 
		   class="${cssClass}" 
		   style="${cssStyle}" lay-verify="${layVerify}"/>
		<a href="javascript:void(0)" class="input-group-addon ${disabled} ${hideBtn ? 'hide' : ''}" id="${id}Button">
	...
	layui.use(["layer"],function(){
		...
		$("#${id}Button, #${id}Name").click(function(){
			...
			layer.open({
				...
				content:"${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}&selectIds="+$("#${id}Id").val(),
